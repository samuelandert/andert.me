      ...
      submitted: {
        invoke: {
          ...
          console.log("submitting");
            const response = await fetch('http://localhost:8080/request_challenge', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json'
              },
              body: JSON.stringify({email: $state.context.formData.email})
            })

            const data = await response.text();
            const getTokenUrl = "http://localhost:8080/login/" + data;
